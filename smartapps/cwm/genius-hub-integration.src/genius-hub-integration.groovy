 ...
 * 
 *  A Hubitat app which integrates with Genius Hub.
 *
 *  ---
 *  Disclaimer:
 *  This app and the associated drivers are in no way sanctioned or supported by Genius Hub.
 *  All work is based on an unpublished api, which may change at any point, causing this app or the
 *  drivers to break. I am in no way responsible for breakage due to such changes. 
 * 
 *  ---
...
definition(
  ...
  description: 'Integrate Genius Hub devices with Hubitat.',
  iconUrl: '',
  iconX2Url: '',
  iconX3Url: '',
  singleInstance: true
...
private void createChildDevice(String deviceType, Integer geniusId, String label) {
  ...
  def device = addChildDevice('cwm', deviceHandler, deviceNetworkId, null, [ 'label': label ])
...
private void pushModeAsync(Integer geniusId, String mode) {
  ...
  asynchttpPatch('updateZoneResponseHandler', requestParams, [ 'geniusId': geniusId ] )
}
...
private void pushSwitchStateAsync(Integer geniusId, Boolean value, Integer overridePeriod) {
  ...
  asynchttpPatch('updateZoneResponseHandler', requestParams, [ 'geniusId': geniusId, updates: ['switchState': value ]] )
}
...
private Map mapRoomUpdates(data) {
  ...
  if (data.containsKey('iBoostTimeRemaining')) {
    updates.overrideEndTime = ((long)(now() / 1000) + data.iBoostTimeRemaining) * 1000
  }
