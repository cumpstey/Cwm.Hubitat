 ...
 * 
 *  A Hubitat smart app which integrates with Zipato.
...
definition(
  ...
  description: 'Integrate Zipato devices with Hubitat.',
  iconUrl: '',
  iconX2Url: '',
  iconX3Url: '',
  singleInstance: true
...
def authenticationPage(params) {
  return dynamicPage(name: 'authenticationPage', title: 'Authentication', install: false, nextPage: 'authenticatedPage') {
    ...
    section {
      ...
      input 'zipaboxLocalIp', 'text', title: 'Local IP address for Zipabox', required: false, displayDuringSetup: true
    }
  }
}
...
private void login() {
  ...
  def requestParams = [
    ...
    path: 'user/login',
    query: [
      'username': settings.zipatoUsername,
      'token': token,
    ],
    contentType: 'application/json',
    headers: [
...
private void pushSwitchStateAsync(String zipatoId, Boolean value) {
  ...
  asynchttpPut('pushSwitchStateResponseHandler', requestParams, [ 'zipatoId': zipatoId, updates: ['switchState': value ]] )
}
...
private getApiRootUrl() {
  return settings.zipaboxLocalIp
    ? "http://${settings.zipaboxLocalIp}:8080/v2/"
    : 'https://my.zipato.com/zipato-web/v2/'
}
