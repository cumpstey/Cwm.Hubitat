...
def actionThermostat(device, attribute, value) {
  switch (attribute) {
    ...
    case "coolingSetpoint":
      device.setCoolingSetpoint(value.toDouble())
      break
    case "heatingSetpoint":
      device.setHeatingSetpoint(value.toDouble())
      break
    ...
    case "thermostatMode":
      ...
      break
    default:
        // Some other commands may be available for a virtual device
        def commandName = "set${attribute.capitalize()}"
        if (device.getSupportedCommands().any {it.name == commandName}) {
            try {
                device."${commandName}"(value)
            } catch (NullPointerException) {
                log.error "The method ${commandName} probably has a non-string strongly typed parameter. It needs to accept a string."
            }
        }
        break;
  }
