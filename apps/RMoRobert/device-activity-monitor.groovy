...
def pageViewReport() {
	dynamicPage(name: "pageViewReport", title: "Device Activity Check", uninstall: false, install: false, nextPage: "pageMain") {
		section(styleSection("Inactive Device Report")) {
         def devices = getInactiveDevices()
         List<com.hubitat.app.DeviceWrapper> activeDevices = devices.active
         List<com.hubitat.app.DeviceWrapper> inactiveDevices = devices.inactive
			if (inactiveDevices) {
...
//interface IDevices {
// List<com.hubitat.app.DeviceWrapper> active;
// List<com.hubitat.app.DeviceWrapper> inactive;
//}

def getInactiveDevices(Boolean sortByName=true) {
   logDebug "getInactiveDevices()...", "trace"
	List<Integer> groups = state.groups ?: [1]
	List<com.hubitat.app.DeviceWrapper> activeDevices = []
   ...
   Closure disabledCheckClosure = { com.hubitat.app.DeviceWrapper dev -> !(dev.isDisabled()) || !(settings["boolIncludeDisabled"]) }
	groups.each { groupNum ->
      ...
      activeDevices.addAll(allDevices - inactiveDevices)
	}
...
void sendInactiveNotification(Boolean includeLastActivityTime=(settings['includeTime'] != false)) {
   log.trace "--"
    ...
    List<com.hubitat.app.DeviceWrapper> activeDevices = devices.active
    List<com.hubitat.app.DeviceWrapper> inactiveDevices = devices.inactive
    logDebug devices.active
    logDebug devices.inactive
	//String notificationText = ""
	if (isModeOK()) {
        activeDevices.each { device ->
            def notification = [
                path: '/push',
                body: [name: device.displayName, value: 'active', type: 'activity']
            ]
            String notificationText = groovy.json.JsonOutput.toJson(notification)
            logDebug notificationText
		    notificationDevice?.deviceNotification(notificationText)
        }
        inactiveDevices.each { device ->
            def notification = [
                path: '/push',
                body: [name: device.displayName, value: 'inactive', type: 'activity']
            ]
            String notificationText = groovy.json.JsonOutput.toJson(notification)
            logDebug notificationText
		    notificationDevice?.deviceNotification(notificationText)
